...
void DallasTemperature::begin(void) {
	...
	while (_wire->search(deviceAddress)) {
		...
		if (validAddress(deviceAddress)) {
			...
			bitResolution = ((bitResolution) > (getResolution(deviceAddress)) ? (bitResolution) : (getResolution(deviceAddress)));
...
bool DallasTemperature::setResolution(const uint8_t* deviceAddress,
	...
	if (isConnected(deviceAddress, scratchPad)) {
		...
		if (deviceAddress[0] != DS18S20MODEL) {
			...
			bitResolution = ((bitResolution)>(newResolution)?(bitResolution):(newResolution));
